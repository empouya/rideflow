...
@Injectable()
export class DriverKafkaEventPublisher
    ...
    constructor(private readonly config: ConfigService) {
        this.publisher = new KafkaEventPublisher(
            buildKafkaConnectionOptions({
                clientId: this.config.get<string>('KAFKA_CLIENT_ID') ?? 'rideflow-driver-service',
    ...
    async onModuleInit(): Promise<void> {
        ...
        logger.info('Driver Kafka publisher connected');
    }

    async onModuleDestroy(): Promise<void> {
        await this.publisher.disconnect();
        logger.info('Driver Kafka publisher disconnected');
    }
